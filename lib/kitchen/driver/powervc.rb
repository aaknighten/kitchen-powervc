    ...
    class Powervc < Kitchen::Driver::Base # rubocop:disable Metrics/ClassLength, Metrics/LineLength
      ...
      required_config :public_key_path do |_, value, driver|
        if value.nil? && driver[:key_name].nil?
          fail(UserError, # rubocop:disable Style/SignalException
      ...
      def find_image(image_ref)
        ...
        fail(ActionFailed, 'Image not found') unless image # rubocop:disable Metrics/LineLength, Style/SignalException
      ...
      def find_flavor(flavor_ref)
        ...
        fail(ActionFailed, 'Flavor not found') unless flavor # rubocop:disable Metrics/LineLength, Style/SignalException
      ...
      def find_network(network_ref)
        ...
        fail(ActionFailed, 'Network not found') unless net # rubocop:disable Metrics/LineLength, Style/SignalException
      ...
      def attach_ip_from_pool(server, pool)
        @@ip_pool_lock.synchronize do
          ...
          if free_addrs.empty?
            fail ActionFailed, "No available IPs in pool <#{pool}>" # rubocop:disable Metrics/LineLength, Style/SignalException
          end
